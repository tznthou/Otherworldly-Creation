...
2. **子集化**為 Big5 全集，共 **13,797 字**。Big5 是變長編碼，兩個區段都涵蓋：
   單位元組區的 ASCII 可見字元 95 個（U+0020–U+007E），加上雙位元組區 13,702 字。
   後者是 Big5 雙位元組區 13,706 字與來源字型 cmap 的交集——來源本身缺其中
   4 個符號 `ˍ‾∼≒`。
   漏掉 ASCII 的話，半形數字與英文會落到系統字型形成混排，補上只多 5.5 KB。
