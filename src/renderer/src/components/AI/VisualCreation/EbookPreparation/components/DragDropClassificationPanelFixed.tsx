...
const DraggableImageItem: React.FC<DraggableImageProps> = ({ image, index, onImageClick }) => {
  ...
  const getImageThumbnail = (img: IllustrationHistoryItem): string => {
    return img.image_url || img.local_file_path || img.image_path || '';
  };
  ...
  return (
    <div
      ...
      className={`
        ...
        ${isDragging
          ? 'opacity-50 scale-110 ring-4 ring-blue-400 ring-opacity-50'
    ...
    >
      ...
      {/* 拖曳狀態指示器 */}
      {isDragging && (
        <div className="absolute inset-0 bg-green-500 bg-opacity-20 border-2 border-green-500 rounded-lg flex items-center justify-center z-40">
...
const DroppableCategoryArea: React.FC<DroppableCategoryProps> = ({
  ...
  return (
    ...
    >
      {/* Debug badge */}
      <div className="absolute -top-6 -left-2 bg-gray-600 text-white text-xs px-2 py-1 rounded z-50 font-mono">
        Zone: {category.id} {isOver && '(HOVER)'}
      </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
            {images.map((image, index) => (
              <div key={`${image.id}-${index}`} className="relative">
                <SafeImage
                  src={image.image_url || image.local_file_path || image.image_path || ''}
...
const DragDropClassificationPanelFixed: React.FC<DragDropClassificationPanelProps> = ({
  selectedImages,
...
export default DragDropClassificationPanelFixed;