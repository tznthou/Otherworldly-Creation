    ...
    scan({
      ...
      onRender: (fiber: unknown, renders: unknown[]) => {
        ...
        if (renderTime > 16) { // 超過一個幀的時間（16ms）
          ...
          console.warn(`⚠️ 組件 "${componentName}" 渲染時間較長: ${renderTime.toFixed(2)}ms`); // TODO: 複雜模式，需人工轉換 // TODO: 複雜模式，需人工轉換
        }
      },
      // 當開始繪製輪廓時的回調
      onPaintStart: (outlines) => {
        if (outlines.length > 10) {
          console.warn(`⚠️ 同時渲染的組件過多: ${outlines.length} 個組件`); // TODO: 複雜模式，需人工轉換 // TODO: 複雜模式，需人工轉換
        }
      },
    });
...
export const monitorComponent = (Component: React.ComponentType<unknown>, componentName: string) => {
  if (process.env.NODE_ENV === 'development') {
    ...
    onRender(Component, (fiber: unknown, render: unknown) => {
      ...
      console.log(`📊 [${componentName}] 渲染時間: ${timeMs}ms`); // TODO: 複雜模式，需人工轉換 // TODO: 複雜模式，需人工轉換
      ...
      if (criticalComponents.includes(componentName) && renderTimeNum > 16) {
        console.warn(`🚨 關鍵組件 ${componentName} 渲染時間過長: ${renderTimeNum.toFixed(2)}ms`); // TODO: 複雜模式，需人工轉換 // TODO: 複雜模式，需人工轉換
      }
    });
  }
};