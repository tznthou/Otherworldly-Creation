...
class ImageGenerationService {
  ...
  async generateImage(
    ...
    try {
      ...
      if (data.predictions && data.predictions.length > 0) {
        ...
        log.debug(`✅ 成功生成 ${results.length} 張圖像`);
        return results;
      }
  ...
  async generateBatch(
    ...
    log.debug(`🚀 開始批次生成 ${requests.length} 張圖像`);
    ...
    for (let i = 0; i < requests.length; i++) {
      ...
      try {
        ...
        log.debug(`📝 處理第 ${i + 1}/${requests.length} 個請求`);
    ...
    log.debug(`🎯 批次生成完成: ${successCount} 成功, ${failCount} 失敗`);
