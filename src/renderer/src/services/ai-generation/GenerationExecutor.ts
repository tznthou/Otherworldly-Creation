...
export class GenerationExecutor {
  ...
  async executeGeneration(config: GenerationConfig): Promise<GenerationResult> {
    ...
    try {
      ...
      log.debug(`✅ 生成成功 [${generationId}]`, { length: filteredText.length });
      return result;

    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      
      log.error(`❌ 生成失敗 [${generationId}]`, error);
  ...
  async executeWithRetry(
    ...
    for (let attempt = 0; attempt <= finalRetryConfig.maxRetries; attempt++) {
      try {
        ...
        if (attempt > 0) {
          ...
          log.debug('⏳ 重試等待', { attempt, delayMs: delay });
        ...
        if (result.success) {
          if (attempt > 0) {
            log.debug('🔄 重試成功', { resultId: result.id, attempt });
          }
          return result;
        }
        ...
        if (!this.isRetryableError(result.error, finalRetryConfig)) {
          log.debug('🚫 不可重試的錯誤', { error: result.error });
          return result;
        }
    ...
    log.error('💥 重試耗盡，最終失敗', { error: lastError });
    return {
      id: `failed-${Date.now()}`,
      text: '',
      temperature: config.params.temperature,
  ...
  async executeBatchGeneration(
    ...
    log.debug('📊 開始批次生成', { count: configs.length });
    ...
    for (let i = 0; i < configs.length; i++) {
      ...
      try {
        ...
        if (result.success) {
          successCount++;
          log.debug('✅ 批次進度 - 成功', { current: i + 1, total: configs.length });
        } else {
          failureCount++;
          if (result.error) errors.push(result.error);
          log.debug('❌ 批次進度 - 失敗', { current: i + 1, total: configs.length, error: result.error });
        ...
        log.error('💥 批次生成未捕獲錯誤', { index: i + 1, error });
    ...
    log.debug('📈 批次生成完成', { successCount, failureCount });
    return batchResult;
  }
