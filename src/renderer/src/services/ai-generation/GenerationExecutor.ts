...
// 創建模組專用 logger
const log = createLogger('GenerationExecutor');
...
export class GenerationExecutor {
  ...
  async executeGeneration(config: GenerationConfig): Promise<GenerationResult> {
    ...
    try {
      console.log(`🚀 開始生成 [${generationId}]:`, { // TODO: 複雜模式，需人工轉換
        model: config.model,
      ...
      console.log(`✅ 生成成功 [${generationId}]: ${filteredText.length} 字符`); // TODO: 複雜模式，需人工轉換
      return result;

    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      
      console.error(`❌ 生成失敗 [${generationId}]:`, error); // TODO: 複雜模式，需人工轉換
  ...
  async executeWithRetry(
    ...
    for (let attempt = 0; attempt <= finalRetryConfig.maxRetries; attempt++) {
      try {
        ...
        if (attempt > 0) {
          ...
          console.log(`⏳ 第${attempt}次重試，等待 ${delay}ms...`); // TODO: 複雜模式，需人工轉換
        ...
        if (result.success) {
          if (attempt > 0) {
            console.log(`🔄 重試成功 [${result.id}] - 第${attempt}次嘗試`); // TODO: 複雜模式，需人工轉換
          }
          return result;
        }
        ...
        if (!this.isRetryableError(result.error, finalRetryConfig)) {
          console.log(`🚫 不可重試的錯誤: ${result.error}`); // TODO: 複雜模式，需人工轉換
          return result;
        }
    ...
    console.error(`💥 重試耗盡，最終失敗: ${lastError}`); // TODO: 複雜模式，需人工轉換
    return {
      id: `failed-${Date.now()}`,
      text: '',
      temperature: config.params.temperature,
  ...
  async executeBatchGeneration(
    ...
    console.log(`📊 開始批次生成 ${configs.length} 個版本`); // TODO: 複雜模式，需人工轉換
    ...
    for (let i = 0; i < configs.length; i++) {
      ...
      try {
        ...
        if (result.success) {
          successCount++;
          console.log(`✅ 批次進度 ${i + 1}/${configs.length} - 成功`); // TODO: 複雜模式，需人工轉換
        } else {
          failureCount++;
          if (result.error) errors.push(result.error);
          console.log(`❌ 批次進度 ${i + 1}/${configs.length} - 失敗: ${result.error}`); // TODO: 複雜模式，需人工轉換
          
          // 檢查是否為配額錯誤，如果是則停止後續生成
          if (this.isQuotaError(result.error)) {
            log.warn('🛑 檢測到配額限制，停止後續生成');
            break;
          }
        }
        ...
        console.error(`💥 批次生成第${i + 1}項發生未捕獲錯誤:`, error); // TODO: 複雜模式，需人工轉換
    ...
    console.log(`📈 批次生成完成: ${successCount}成功, ${failureCount}失敗`); // TODO: 複雜模式，需人工轉換
    return batchResult;
  }
