...
export const useVisualCreationHandlers = () => {
  ...
  const handleProviderChange = useCallback((provider: IllustrationProvider) => {
    ...
    console.log(`🔄 插畫服務切換至: ${provider === 'pollinations' ? 'Pollinations.AI (免費)' : 'Google Imagen (付費)'}`); // TODO: 複雜模式，需人工轉換
  }, [dispatch]);

  // 標籤切換處理
  const handleTabChange = useCallback((tab: ActiveTab) => {
    dispatch(setActiveTab(tab));
    console.log(`🎯 切換至標籤頁: ${tab}`); // TODO: 複雜模式，需人工轉換
  }, [dispatch]);
  ...
  const handleSaveSelectedImages = useCallback(async (imageIds: string[]) => {
    ...
    try {
      ...
      // 添加成功通知
      console.log(`✅ 成功保存 ${imageIds.length} 張圖片到圖庫！`); // TODO: 複雜模式，需人工轉換
