...
// 創建模組專用 logger
const log = createLogger('useVisualCreationData');
...
export const useVisualCreationData = () => {
  ...
  useEffect(() => {
    if (currentProject) {
      log.debug('🎨 [VisualCreationCenter] 初始化，專案ID:', currentProject.id);
      dispatch(clearError());
      dispatch(initializeVisualCreation(currentProject.id));
      
      // 載入角色資料到 Redux
      log.debug('📊 [VisualCreationCenter] 載入角色資料...');
  ...
  // 調試：監控角色狀態變化
  useEffect(() => {
    log.debug('🎨 [VisualCreationCenter] 角色狀態更新:');
    log.debug('   📊 角色總數:', characters.length);
    if (characters.length > 0) {
      log.debug('   🎭 角色列表:', characters.map(c => ({
        id: c.id,
        name: c.name,
    ...
    if (currentProject) {
      ...
      log.debug('   🎯 專案角色數:', projectCharacters.length);
    }
