...
export function useAutoVersionCreation() {
  ...
  const createVersionForImage = useCallback(async (imageId: string) => {
    ...
    try {
      ...
      if (result.success) {
        log.debug('✅ 自動創建版本成功', { imageId, versionId: result.versionId });
      ...
      log.error('❌ 自動創建版本失敗', { imageId, error });
  ...
  const manuallyCreateVersion = useCallback(async (
    ...
    try {
      ...
      if (result.success) {
        log.debug('✅ 手動創建版本成功', { imageId, versionId: result.versionId });
      ...
      log.error('❌ 手動創建版本失敗', { imageId, error });
      throw error;
    }
  }, [dispatch, tempImages, versions]);
  ...
  useEffect(() => {
    ...
    const processPendingVersions = async () => {
      log.debug('🔄 開始自動處理待創建版本', { count: pendingVersionCreation.length });
  ...
  useEffect(() => {
    if (lastGeneratedImageId && autoCreateVersions) {
      log.debug('🎨 新圖片生成完成', { lastGeneratedImageId });
    }
