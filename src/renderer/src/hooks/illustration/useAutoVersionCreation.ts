...
export function useAutoVersionCreation() {
  ...
  const createVersionForImage = useCallback(async (imageId: string) => {
    ...
    if (!tempImage) {
      console.warn(`找不到臨時圖片 ID: ${imageId}`); // TODO: 複雜模式，需人工轉換 // TODO: 複雜模式，需人工轉換
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        console.log(`✅ 自動創建版本成功：圖片 ${imageId} → 版本 ${result.versionId}`); // TODO: 複雜模式，需人工轉換 // TODO: 複雜模式，需人工轉換
      ...
      console.error(`❌ 自動創建版本失敗：圖片 ${imageId}`, error); // TODO: 複雜模式，需人工轉換 // TODO: 複雜模式，需人工轉換
  ...
  const manuallyCreateVersion = useCallback(async (
    ...
    try {
      ...
      if (result.success) {
        console.log(`✅ 手動創建版本成功：圖片 ${imageId} → 版本 ${result.versionId}`); // TODO: 複雜模式，需人工轉換 // TODO: 複雜模式，需人工轉換
      ...
      console.error(`❌ 手動創建版本失敗：圖片 ${imageId}`, error); // TODO: 複雜模式，需人工轉換 // TODO: 複雜模式，需人工轉換
      throw error;
    }
  }, [dispatch, tempImages, versions]);
  ...
  useEffect(() => {
    ...
    const processPendingVersions = async () => {
      console.log(`🔄 開始自動處理 ${pendingVersionCreation.length} 個待創建版本...`); // TODO: 複雜模式，需人工轉換 // TODO: 複雜模式，需人工轉換
  ...
  useEffect(() => {
    if (lastGeneratedImageId && autoCreateVersions) {
      console.log(`🎨 新圖片生成完成：${lastGeneratedImageId}，即將自動創建版本...`); // TODO: 複雜模式，需人工轉換 // TODO: 複雜模式，需人工轉換
    }
