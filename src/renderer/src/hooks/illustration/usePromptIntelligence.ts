...
export function usePromptIntelligence(
  ...
  const generateSuggestions = useCallback(async (
    ...
    try {
      ...
      log.debug(`🤖 [PromptIntelligence] 生成 ${suggestions.length} 個建議`);
      return suggestions;
  ...
  const analyzePrompt = useCallback(async (
    ...
    try {
      ...
      log.debug(`🔍 [PromptIntelligence] 分析完成: ${analysis.confidence * 100}% 信心度`);
      return analysis;
  ...
  const applySuggestion = useCallback((suggestion: PromptSuggestion) => {
    log.debug(`✨ [PromptIntelligence] 應用建議: ${suggestion.text}`);
