...
export function useBatchExportProcessor(
  ...
  const addImageToExport = useCallback((imageId: string, imageUrl: string, fileName?: string) => {
    ...
    log.debug(`📥 [BatchExportProcessor] 添加圖片到導出佇列: ${imageId}`);
  }, [exportManager]);
  ...
  const addMultipleImages = useCallback((images: Array<{ id: string; url: string; fileName?: string }>) => {
    ...
    log.debug(`📥 [BatchExportProcessor] 批次添加 ${images.length} 張圖片到導出佇列`);
  }, [exportManager]);
  ...
  const processCurrentQueue = useCallback(async () => {
    ...
    try {
      ...
      log.debug(`🚀 [BatchExportProcessor] 開始處理 ${pendingTasks.length} 個任務`);
      ...
      log.debug(`✅ [BatchExportProcessor] 批次處理完成: ${completedCount} 成功, ${failedCount} 失敗`);
